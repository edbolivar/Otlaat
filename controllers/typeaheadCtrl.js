app.controller('TypeaheadCtrl', function ($scope, $http, HotelSearchService, $timeout, $rootScope, constants) {
    ...
    $scope.getDestinations = function (search) {
        var params = {searchParam: search};
        $scope.showLocationSpinner = true;

        return HotelSearchService.getLocation(params).then(function (response) {
             if (response.data.success) {
                 var destinationResults = []; 
                 angular.forEach(response.data.autoSearchRes, function (itemParent, locationNameParent) {
                    angular.forEach(itemParent, function(item, locationName){
                         var result = angular.extend(item, {name: locationName});

                         if (destinationResults.length < 30) {
                             destinationResults.push(result);
                         }
                    });
                 });
                 $rootScope.showErrorDestinationMsg = false;
                 $scope.showErrorMsg = false;
                 $scope.showLocationSpinner = false;
                 //console.log('dd', destinationResults);
                 return destinationResults;
             } else {
                 //debugger
                 $scope.showLocationSpinner = false;
                 $scope.showErrorMsg = true;
                 $scope.errorMsg = response.data.err_msg;
             }
         });
    ...
    $scope.$watch('typeAheadIsOpen', function (newVal, oldVal) {
        ...
        if (newVal) {
            var searchField = angular.element.find('.location-autocomplete'),
                width = searchField[0].offsetWidth,
                dropDowns = angular.element.find('.dropdown-menu'),
                dropDown;
            angular.forEach(dropDowns, function (dropdown) {
                if (dropdown.id && dropdown.id.indexOf('typeahead') > -1) {
                    dropDown = dropdown;
                }
            });
            if (dropDown) {
                $timeout(function () {
                    angular.element(dropDown).css('width', (width + 'px'));
                });
            }
